...
class AsyncLinkProcessor:
    ...
    async def process_link(self, url: str, session: aiohttp.ClientSession) -> Dict[str, Any]:
        ...
        try:
            ...
            creation_date = soup.select_one('span.date').text.strip()
            print(creation_date)
            article_title = soup.select_one('span.field--name-title').text.strip() 
            print(article_title)
            article_pretext = soup.select_one('div.quote-wrapper').text.strip() if soup.select_one('div.quote-wrapper').text.strip() else ''
            print(article_pretext)
            article_maintext = soup.select_one('div.body').text.strip()
